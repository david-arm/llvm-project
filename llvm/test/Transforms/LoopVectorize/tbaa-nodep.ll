; RUN: opt < %s  -aa-pipeline=tbaa,basic-aa -passes=loop-vectorize,simplifycfg -force-vector-interleave=1 -force-vector-width=4 -simplifycfg-require-and-preserve-domtree=1 -S | FileCheck %s
; RUN: opt < %s  -aa-pipeline=basic-aa -passes=loop-vectorize,simplifycfg -force-vector-interleave=1 -force-vector-width=4 -simplifycfg-require-and-preserve-domtree=1 -S | FileCheck %s --check-prefix=CHECK-NOTBAA
